...
| **FR-23** | PS-13 | **Grounded question-answering over the system's own numbers.** A tool-calling agent behind `POST /api/chat` that answers from the baked payloads and this project's own documents, never from recall. Three guards, in order of importance: a **numeric guard** rejects any figure that did not come back from a tool call; a **refusal table** blocks the questions the data cannot honestly answer (casualty counts, “is this validated?”); and advisory text is substituted **verbatim**, never paraphrased, because a reworded public health instruction is a new instruction nobody approved. Retrieval is hybrid and local. Tools grow as FR-16–FR-18 land: `get_lst_anomaly`, `get_population_exposure`, `get_model_card`, `run_siting`. The panel hides itself when the API is unreachable, so NFR-1 is untouched | P2 `[P5]` | 🚧 planned — Phase 5G, ~13 h |
