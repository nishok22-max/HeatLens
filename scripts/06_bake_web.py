...
def main(config_path: str) -> None:
    ...
    # D15: exposure and vulnerability get SEPARATE rows. They are sourced
    # differently and are at different standards, and a single row carrying
    # both let the measured half borrow credibility from nothing.
    exposure_measured = bool(cube["exposure_is_measured"])         if "exposure_is_measured" in cube.files else False
    exposure_note = str(cube["exposure_provenance"])         if "exposure_provenance" in cube.files else ""

    if exposure_measured:
        exposure_row = {
            "layer": "population",
            "plain": "How many people live there",
            "source": exposure_note or "WorldPop",
        ...
        exposure_row = {
            ...
            "status": "NOT MEASURED -- proxy is a rescale of the heat pattern, "
                      "so risk cannot reorder zones relative to hazard",
        }
